...
def plot_lidar_data(sim, lidar_handle):
    """
    Lee y grafica los datos del LIDAR.
    """
    angles, distances = read_lidar_data(sim, lidar_handle)
    if angles is None or distances is None:
        print("No hay datos de LIDAR para graficar.")
        return
    plt.figure()
    plt.polar(angles, distances, '.')
    plt.title("Datos LIDAR Hokuyo")
    plt.show()
...
def main():
    ...
    handles = sim_env.get_handles({
        ...
        'lidar': '/PioneerP3DX/fastHokuyo', # Handle del LIDAR
    ...
    try:
        ...
        # Plottear datos del lidar
        plot_lidar_data(sim_env.sim, handles['lidar'])
